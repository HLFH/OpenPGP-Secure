...
N.B: `python2-unbound` dependency of `hash-slinger` is really heavy because it depends also of the `unbound` recursive DNS server package. So, it's now better [to generate your OPENPGPKEY RR with `gpg`](https://tools.ietf.org/html/rfc7929#appendix-A).
